...
//   // Load cart and merge Buy Now item
//   useEffect(() => {
...
//         // If Buy Now, merge with cart temporarily
//         if (buyNowItem) {
...
//     toast.success(`Payment of ₹${totalAmount} successful via ${paymentMethod.toUpperCase()}`);

//     const orderData = { items: cartItems, total: totalAmount, billing: billingInfo, paymentMethod };
//     localStorage.setItem("lastOrder", JSON.stringify(orderData));
...
//                     <span className="font-semibold text-gray-800">{item.name}</span>
//                     <div className="flex items-center gap-2 mt-1">
//                       <button onClick={() => updateQuantity(item.id, -1)} className="bg-gray-200 px-3 py-1 rounded hover:bg-gray-300 transition">-</button>
//                       <span className="px-3">{item.quantity}</span>
//                       <button onClick={() => updateQuantity(item.id, 1)} className="bg-gray-200 px-3 py-1 rounded hover:bg-gray-300 transition">+</button>
//                     </div>
//                   </div>
...
//             ))}
//             <div className="flex gap-2 mt-4">
//               <input type="text" placeholder="Enter coupon code" value={coupon} onChange={(e) => setCoupon(e.target.value)} className="border border-gray-300 p-2 rounded flex-1"/>
//               <button onClick={applyCoupon} className="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 transition">Apply</button>
//             </div>
//             <div className="mt-4 border-t pt-4 space-y-2 text-gray-700">
//               <div className="flex justify-between">
//                 <span>Subtotal:</span>
//                 <span>₹{cartItems.reduce((acc, item) => acc + item.quantity * parseFloat(item.price.replace(/[^\d.]/g, "")), 0).toFixed(2)}</span>
//               </div>
//               <div className="flex justify-between">
//                 <span>Discount:</span>
//                 <span>₹{discount.toFixed(2)}</span>
